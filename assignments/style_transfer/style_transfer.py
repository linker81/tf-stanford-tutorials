...
IMAGE_HEIGHT = 250	# The dimension of the images affects drammatically the feasibility of the computation.
IMAGE_WIDTH = 333	# If you receive a Run Out Memory error, reduce these dimensions
...
DATA_DIR = "/Tmp/data/VGG/"
LOG_DIR = "/Tmp/my_graph/a2/"

VGG_DOWNLOAD_LINK = 'http://www.vlfeat.org/matconvnet/models/imagenet-vgg-verydeep-19.mat'
VGG_MODEL = DATA_DIR + 'imagenet-vgg-verydeep-19.mat'
...
def _create_content_loss(p, f):
    ...
    """
    coeff = 1.0/(4.0 * p.size)
    loss_content = coeff * tf.reduce_sum((f - p)**2)
    return loss_content
...
def _single_style_loss(a, g):
    ...
    """
    N = a.shape[3] # Number of features
    M = a.shape[1] * a.shape[2] # Height x Width of the feature map
    G = _gram_matrix(g, N, M)
    A = _gram_matrix(a, N, M)

    E_l = tf.reduce_sum((G - A) ** 2) / ((2.0 * N * M) ** 2)
    return E_l
...
def _create_style_loss(A, model):
    ...
    #Compute the loss style for each Layer
    ...
    for i in range(n_layers):
        total_style_loss += tf.reduce_sum(W[i] * E[i])
...
def train(model, generated_image, initial_image):
    ...
    with tf.Session(config = config) as sess:
        ...
        writer = tf.summary.FileWriter(LOG_DIR + 'styletransfer', sess.graph)

        sess.run(generated_image.assign(initial_image))
        ckpt = tf.train.get_checkpoint_state(os.path.dirname(LOG_DIR + 'checkpoints/checkpoint'))
        if False and ckpt and ckpt.model_checkpoint_path:
        ...
        for index in range(initial_step, ITERS):
            ...
            if (index + 1) % skip_step == 0:
                ...
                if (index + 1) % 20 == 0:
                    saver.save(sess, LOG_DIR + 'checkpoints/style_transfer', index)
...
def main():
    ...
    if not (os.access(VGG_MODEL, os.R_OK)):
